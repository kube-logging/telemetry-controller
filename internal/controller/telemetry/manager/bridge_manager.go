...
func (b *BridgeManager) GetBridgesForTenant(ctx context.Context, tenantName string) (bridgesOwned []v1alpha1.Bridge, err error) {
	listOpts := &client.ListOptions{
		FieldSelector: fields.OneTermEqualSelector(resources.BridgeSourceTenantReferenceField, tenantName),
	}
	...
	listOpts = &client.ListOptions{
		FieldSelector: fields.OneTermEqualSelector(resources.BridgeTargetTenantReferenceField, tenantName),
	}
...
func (b *BridgeManager) CheckBridgeConnection(ctx context.Context, tenantName string, bridge *v1alpha1.Bridge) error {
	for _, tenantReference := range []string{bridge.Spec.SourceTenant, bridge.Spec.TargetTenant} {
		if tenantReference != tenantName {
			listOpts := &client.ListOptions{
				FieldSelector: fields.OneTermEqualSelector(resources.TenantNameField, tenantReference),
			}
